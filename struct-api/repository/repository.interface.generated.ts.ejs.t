...
export abstract class <%= struct.name.pascalName %>RepositoryInterfaceGenerated {
  // 汎用検索系メソッド
  abstract getAll(condition: <%= struct.name.pascalName %>Entity): Promise<<%= struct.name.pascalName %>Entity[]>;
  abstract count(condition: <%= struct.name.pascalName %>Entity): Promise<number>;
  abstract getAllWithCursor(condition: <%= struct.name.pascalName %>Entity): Promise<<%= struct.name.pascalName %>Entity[]>;
  // CRUD系メソッド
